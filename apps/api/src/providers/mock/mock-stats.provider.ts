...
import type { IStatsProvider, RawStats } from '../external-stats-provider.interface'

@Injectable()
export class MockStatsProvider implements IStatsProvider {
  async fetchStats(_userId: string, _gameSlug: string): Promise<RawStats> {
    ...
    return { kills, deaths, wins, losses, matchesPlayed, playtimeMinutes }
  }
